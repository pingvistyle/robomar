module Robomar
  ...
  class Robot
    ...
    def initialize(area, x, y, direction)
      @x = x.to_i
      @y = y.to_i    
      raise IncorrectionPositionRobot if @x < 0 || @y < 0

      @direction = direction.to_sym
      raise IncorrectionDirectionRobot unless DIRECTIONS.include?(@direction)

      @area = area    
      raise OutareaRobot if outarea?
    end
  ...
  public
    ...
    def programming(commands)
      raise IncorrectlyCommandRobot unless commands.scan(/[^MRL]+/).empty?
